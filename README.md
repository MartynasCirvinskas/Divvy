...
| `npm run smoke` | End-to-end smoke test against your real Firebase project (requires `.env`) |

## Building

This project uses **Continuous Native Generation (CNG)** — `android/` and
`ios/` are not committed; they're generated by `expo prebuild` at build
time. All native config lives in `app.config.js`.

### Cloud build via EAS (recommended — works on any OS)
...
npx eas-cli init             # creates an Expo project + writes the projectId
```
...
|---|---|---|
| `npm run build:dev` | development | Dev client APK / IPA — for live-reload dev work |
| `npm run build:preview:android` | preview | Installable APK (sideload to phone) |
| `npm run build:preview:ios` | preview | Ad-hoc IPA (requires Apple Developer account, $99/yr) |
| `npm run build:prod` | production | AAB for Play Store + IPA for App Store |

Free tier is **30 builds/month** combined — plenty for indie pace.

### Local build (requires native toolchain)

If you have Android Studio + JDK 17 (or Xcode on macOS):

```bash
npm run prebuild     # regenerates android/ and ios/
npm run android      # builds + installs on a connected device or emulator
